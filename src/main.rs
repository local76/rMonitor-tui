...
impl App {
    fn new() -> Self {
        ...
        gpu_names.sort_by(|a, b| {
            let is_discrete_a =
                a.contains("RX") || a.contains("RTX") || a.contains("GTX") || a.contains("NVIDIA");
            let is_discrete_b =
    ...
    fn update_metrics(&mut self) {
        ...
        self.processes = self
            ...
            .iter()
            .map(|(pid, p)| {
                let scaled_cpu = if num_cpus > 0.0 {
...
fn main() -> Result<(), io::Error> {
    ...
    std::panic::set_hook(Box::new(|info| {
        ...
        eprintln!(
            "rMonitor crashed! Panic logged. Error: {} at {}",
            msg, location
        );
    }));
...
fn draw_ui(f: &mut ratatui::Frame, app: &mut App) {
    ...
    let cpu_border_color = if app.focus == FocusedSection::Cpu {
        theme.accent
    } else {
        theme.border
    };
    let mem_border_color = if app.focus == FocusedSection::Memory {
        theme.accent
    } else {
        theme.border
    };
    let disk_border_color = if app.focus == FocusedSection::Disk {
        theme.accent
    } else {
        theme.border
    };
    let gpu_border_color = if app.focus == FocusedSection::Gpu {
        theme.accent
    } else {
        theme.border
    };
    let net_border_color = if app.focus == FocusedSection::Network {
        theme.accent
    } else {
        theme.border
    };
    ...
    let (text_color, status_text) = if app.status_msg == "Press Tab to cycle panel focus" {
        (
            theme.text_dim,
            "Ready. Press Tab to cycle focus.".to_string(),
        )
    } else {
...
fn draw_context_table(f: &mut ratatui::Frame, area: Rect, app: &mut App) {
    ...
    match app.focus {
        ...
        FocusedSection::Memory => {
            ...
            let mem_stats = vec![
                ...
                (
                    "Total Pagefile Swap (Swapfile)",
                    format!("{:.2} GB", total_swap),
                ),
                (
                    "Used Pagefile Swap (Swapfile)",
                    format!("{:.2} GB", used_swap),
                ),
                (
                    "Free Pagefile Swap (Swapfile)",
                    format!("{:.2} GB", free_swap),
                ),
            ];
        ...
        FocusedSection::Disk => {
            ...
            for disk in &app.disks {
                ...
                rows.push(Row::new(vec![
                    Cell::from(disk.mount_point().to_string_lossy().to_string()).style(
                        Style::default()
                            .fg(theme.text_main)
                            .add_modifier(Modifier::BOLD),
                    ),
                    ...
                    Cell::from(format!("{:.1} GB", used))
                        .style(Style::default().fg(Color::Rgb(160, 32, 240))),
                    Cell::from(format!("{:.1} GB", total))
                        .style(Style::default().fg(theme.text_dim)),
                    Cell::from(format!("{:.1} GB", avail))
        ...
        .map(|p| {
            ...
            let net_style = if app.focus == FocusedSection::Network {
                Style::default()
                    ...
                    .add_modifier(Modifier::BOLD) // Electric Cyan
            } else {
                Style::default().fg(theme.text_main)
            };
    ...
    let headers = vec![
        ...
        Cell::from("CPU").style(if app.focus == FocusedSection::Cpu {
            Style::default()
                .fg(Color::Rgb(80, 250, 123))
                .add_modifier(Modifier::BOLD)
        } else {
        ...
        Cell::from("Memory").style(if app.focus == FocusedSection::Memory {
            Style::default()
                .fg(Color::Rgb(255, 121, 198))
                .add_modifier(Modifier::BOLD)
        } else {
        ...
        Cell::from("Storage").style(if app.focus == FocusedSection::Disk {
            Style::default()
                .fg(Color::Rgb(160, 32, 240))
                .add_modifier(Modifier::BOLD)
        } else {
        ...
        Cell::from("GPU").style(if app.focus == FocusedSection::Gpu {
            Style::default()
                .fg(Color::Rgb(255, 215, 0))
                .add_modifier(Modifier::BOLD)
        } else {
        ...
        Cell::from("Network").style(if app.focus == FocusedSection::Network {
            Style::default()
                .fg(Color::Rgb(0, 245, 255))
                .add_modifier(Modifier::BOLD)
        } else {
...
fn format_gpu_line(label: &str, name: &str, pct: f64, width: usize) -> Line<'static> {
    ...
    Line::from(vec![
        Span::styled(
            format!("{}: ", label),
            Style::default().fg(Color::Rgb(136, 136, 153)),
        ),
...
fn print_json_snapshot() {
    ...
    let os_name = System::long_os_version()
        .unwrap_or_else(|| System::name().unwrap_or_else(|| "Windows".to_string()));
    let host_name = std::env::var("COMPUTERNAME").unwrap_or_else(|_| "localhost".to_string());
    let username = std::env::var("USERNAME")
        .unwrap_or_else(|_| std::env::var("USER").unwrap_or_else(|_| "user".to_string()));
    ...
    for (name, data) in &networks {
        ...
        net_info.insert(
            "mac".to_string(),
            serde_json::Value::String(data.mac_address().to_string()),
        );
        net_info.insert(
    ...
    root.insert(
        "kernel".to_string(),
        serde_json::Value::String(System::kernel_version().unwrap_or_default()),
    );
    ...
    root.insert(
        ...
        serde_json::Value::Object(net_interfaces),
    );
...
fn run_doctor() {
    ...
    let clip_ok = copy_text_to_clipboard("rMonitor Diagnostic Test").is_ok();
    println!(
        "Windows Clipboard: {}",
        if clip_ok {
            "Accessible (OK)"
...
impl App {
    fn open_embedded_markdown(&mut self, title: &str, content: &str, theme: &ThemeColors) {
        self.markdown_lines = parse_markdown_to_lines(content, theme);
...
fn parse_markdown_to_lines(content: &str, theme: &ThemeColors) -> Vec<ratatui::text::Line<'static>> {
